...
function zoneNameOf(order) {
  const zone = order.provider?.zone
  if (zone?.zone_name) {
    return zone.city ? `${zone.zone_name} · ${zone.city}` : zone.zone_name
  }
  return order.zone_name ?? null
}
...
function AnalyticsServiceOrders() {
  ...
  useEffect(() => {
    if (queryName || !serviceId || serviceId === 'unknown') return
    getService(serviceId)
      .then((raw) => setName(raw.name_en ?? raw.name ?? 'Service'))
      .catch(() => {})
  }, [serviceId, queryName])
  ...
  return (
    <div className="dt-page-layout">
      <PageHeader
        title={name || 'Service orders'}
        subtitle={`Orders for this service · ${rangeLabel}`}
      ...
      <div className="dt-card">
        {loading ? (
          ...
          <div className="dt-state">No orders for this service in this range.</div>
        ) : (
          <TableScroll>
...
export default AnalyticsServiceOrders
