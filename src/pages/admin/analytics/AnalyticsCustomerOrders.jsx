...
import { getCustomerDetails } from '../../../api/customers.js'
...
function AnalyticsCustomerOrders() {
  ...
  useEffect(() => {
    if (queryName || !customerId) return
    getCustomerDetails(customerId)
      .then((raw) => setName(
        raw.user?.full_name
        ?? raw.profile?.full_name
        ?? raw.profile?.company_name
        ?? raw.full_name
        ?? 'Customer',
      ))
      .catch(() => {})
  }, [customerId, queryName])
  ...
  return (
    <div className="dt-page-layout">
      <PageHeader
        title={name || 'Customer orders'}
        subtitle={`Orders for this customer · ${rangeLabel}`}
      ...
      <div className="dt-card">
        {loading ? (
          ...
          <div className="dt-state">No orders for this customer in this range.</div>
        ) : (
          <TableScroll>
...
export default AnalyticsCustomerOrders
